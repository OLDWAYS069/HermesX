...
#include "HermesXZhuyinSingleStatic.h"
#else
#include "HermesXZhuyinSingleStatic.h"
#endif
#include <algorithm>
...
// Everything written by hand lives in namespace hermesx_bpmf so this layer can be
...
inline void emit_node(int node, std::vector<HermesXBpmfCandidate> &out, int max_out, bool completion = false)
{
...
// weights to sort on.
inline void collect_subtree(int node, std::vector<HermesXBpmfCandidate> &out, int max_out, int depth)
{
...
inline void dfs_walk(const char *full, int len, int pos, int node, int depth,
                     ...
                     std::vector<HermesXBpmfCandidate> &out, int max_out, bool allow_init)
{
    if ((int)out.size() >= max_out)
        return;
...
inline std::vector<std::string> unified_search(const std::string &input, int max_out = 20)
{
    ...
    std::vector<HermesXBpmfCandidate> cands;
    ...
    if (wantTone)
        std::stable_partition(cands.begin(), cands.end(),
                              [&](const HermesXBpmfCandidate &c) { return (c.tone & wantTone) != 0; });
    noTone = (wantTone == 0);
#endif
    ...
    if (noTone)
        std::stable_partition(cands.begin(), cands.end(), [](const HermesXBpmfCandidate &c) {
