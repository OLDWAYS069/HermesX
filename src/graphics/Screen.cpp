...
uint32_t logo_timeout = 2000; // 約 1 秒官方頁 + 1 秒 HermesX 頁

// Boot splash timing/state
static bool bootScreenShowing = true;
static bool bootScreenSwitched = false;
static uint32_t bootScreenStartMs = 0;
static uint32_t bootScreenSwitchDueMs = 0;
static uint32_t bootScreenEndDueMs = 0;
static uint32_t bootScreenSwitchEffectiveMs = 0;
...
static __attribute__((unused)) bool shouldDrawMessage(const meshtastic_MeshPacket *packet)
{
...
void Screen::setup()
{
    ...
    alertFrames[0] = [this](OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x, int16_t y) {
...
#endif
        {
            ...
            graphics::UIRenderer::drawIconScreen(region, display, state, x, y); // 官方開機頁
        }
    };
    ui->setFrames(alertFrames, 1);
    ui->disableAutoTransition(); // Require manual navigation between frames
    bootScreenShowing = true;
    bootScreenSwitched = false;
    bootScreenStartMs = millis();
    uint32_t half = logo_timeout / 2;
    bootScreenSwitchDueMs = bootScreenStartMs + half;
    bootScreenEndDueMs = bootScreenSwitchDueMs + half;
    bootScreenSwitchEffectiveMs = 0;
...
int32_t Screen::runOnce()
{
    ...
    // Show boot screen for first logo_timeout milliseconds, then switch to normal operation.
    const uint32_t nowMs = millis();
    const uint32_t halfTimeout = logo_timeout / 2;

    // 半程切換至 HermesX 開機畫面（先顯示官方，再顯示 HermesX）
    if (bootScreenShowing && !bootScreenSwitched && nowMs >= bootScreenSwitchDueMs) {
        LOG_INFO("Switching to HermesX boot screen (elapsed=%ums of %ums)", nowMs - bootScreenStartMs, logo_timeout);
        alertFrames[0] = graphics::UIRenderer::drawHermesXBootScreen;
        ui->setFrames(alertFrames, 1);
        ui->update();
#ifndef USE_EINK
        ui->update();
#endif
        bootScreenSwitched = true;
        bootScreenSwitchEffectiveMs = nowMs;
        bootScreenEndDueMs = nowMs + halfTimeout; // 保證 HermesX 頁顯示完整半段
    }

    // 確保 HermesX 頁至少顯示半段時間
    if (bootScreenShowing && bootScreenSwitched && nowMs >= bootScreenEndDueMs) {
        LOG_INFO("Done with boot screen (total=%ums, hermes=%ums)", nowMs - bootScreenStartMs, nowMs - bootScreenSwitchEffectiveMs);
        stopBootScreen();
        bootScreenShowing = false;
    } else if (bootScreenShowing && !bootScreenSwitched && nowMs >= bootScreenEndDueMs) {
        // Fallback: if尚未切到 HermesX 也不要卡住
        LOG_INFO("Done with boot screen (fallback total=%ums)", nowMs - bootScreenStartMs);
        stopBootScreen();
        bootScreenShowing = false;
    }
    ...
    for (;;) {
        ...
        case Cmd::START_ALERT_FRAME: {
            bootScreenShowing = false; // this should avoid the edge case where an alert triggers before the boot screen goes away
