...
int SystemCommandsModule::handleInputEvent(const InputEvent *event)
{
    ...
    // 未特別處理的輸入，僅消除 -Wswitch-enum 警告
    case INPUT_BROKER_NONE:
    ...
    case INPUT_BROKER_SHUTDOWN: {
        // 先跑 HermesX 的關機動畫，稍微延遲再進入 shutdown
        if (HermesXInterfaceModule::instance) {
            // 停掉 power-hold 逐格，讓關機動畫優先
        ...
        constexpr uint32_t kShutdownDelayMs = 1200; // 確保動畫能顯示
        shutdownAtMsec = millis() + kShutdownDelayMs;
        return true;
    }
