...
int CannedMessageModule::handleInputEvent(const InputEvent *event)
{
    ...
    if (event->inputEvent == static_cast<char>(meshtastic_ModuleConfig_CannedMessageConfig_InputEventChar_SELECT)) {
        ...
        if (this->currentMessageIndex == 0) {
            ...
            requestCannedFocus(); // Tell Screen::setFrames to move to our module's frame, next time it runs
            UIFrameEvent e;
            e.action = UIFrameEvent::Action::REGENERATE_FRAMESET; // We want to change the list of frames shown on-screen
            notifyUiFrameEvent(e);
...
int CannedMessageModule::getCurrentMessageIndex() const
{
...
#if !MESHTASTIC_EXCLUDE_HERMESX
        if (hermesActive) {
            requestCannedFocus(); // Tell Screen::setFrames to move to our module's frame
...
ProcessMessage CannedMessageModule::handleReceived(const meshtastic_MeshPacket &mp)
{
    if (mp.decoded.portnum == meshtastic_PortNum_ROUTING_APP && waitingForAck) {
        ...
        if (mp.decoded.request_id != 0) {
            UIFrameEvent e;
            e.action = UIFrameEvent::Action::REGENERATE_FRAMESET; // We want to change the list of frames shown on-screen
            requestCannedFocus(); // Tell Screen::setFrames that our module's frame should be shown, even if not "first" in the frameset
...
#if !MESHTASTIC_EXCLUDE_HERMESX
size_t CannedMessageModule::getEmergencyMenuEntryCount() const
{
    if (activeList == CannedListKind::NORMAL) {
        return 0;
    }
    return emergencyMenu.size();
}

bool CannedMessageModule::getEmergencyMenuEntry(size_t index, const char *&label, meshtastic_EmergencyType &type) const
{
    if (activeList == CannedListKind::NORMAL || index >= emergencyMenu.size()) {
        label = "";
        type = meshtastic_EmergencyType_SOS;
        return false;
    }
    ...
    case EmergencyMenuOption::Action::SAFE:
        type = meshtastic_EmergencyType_SAFE;
        break;
    case EmergencyMenuOption::Action::NEED:
        type = meshtastic_EmergencyType_NEED;
        break;
    case EmergencyMenuOption::Action::RESOURCE:
        type = meshtastic_EmergencyType_RESOURCE;
        break;
    }
    return true;
}
...
void CannedMessageModule::requestCannedFocus()
{
    MeshModule::requestFocus();
}

void CannedMessageModule::notifyUiFrameEvent(UIFrameEvent &evt)
{
...
bool CannedMessageModule::handleEmergencySelection(size_t index)
{
    ...
    const auto &entry = emergencyMenu[index];
    meshtastic_EmergencyType type = meshtastic_EmergencyType_SOS;
    switch (entry.action) {
    case EmergencyMenuOption::Action::SOS:
        type = meshtastic_EmergencyType_SOS;
        break;
    ...
    HermesXInterfaceModule *hermes = HermesXInterfaceModule::instance;
    if (hermes) {
        hermes->beginEmergencyAction(type);
    }
    ...
    case EmergencyMenuOption::Action::SOS:
        ...
        if (ok && hermes) {
            hermes->playSOSFeedback();
        }
        break;
    case EmergencyMenuOption::Action::SAFE:
        ok = emergencyModule->sendSafe();
        if (ok && hermes) {
            hermes->showEmergencyBanner(true, F("SAFE SENDING"), 0x07E0);
            hermes->playAckSuccess();
        }
        break;
    case EmergencyMenuOption::Action::NEED:
        ok = emergencyModule->sendNeed(entry.codes[0]);
        if (ok && hermes) {
            hermes->playSendFeedback();
        }
        break;
    case EmergencyMenuOption::Action::RESOURCE:
    ...
    if (!ok && hermes) {
        hermes->markEmergencyActionFailed(type);
    }
    return ok;
#else
    (void)index;
