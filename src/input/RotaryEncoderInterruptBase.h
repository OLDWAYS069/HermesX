...
class RotaryEncoderInterruptBase : public Observable<const InputEvent *>, public concurrency::OSThread
{
  ...
  private:
    ...
    // Raise the long-press threshold to reduce spurious short-press resets on noisy EC11 buttons
    static const uint32_t LONG_PRESS_DURATION = 5000;     // ms (拉長至 5 秒對應關機逐格動畫)
