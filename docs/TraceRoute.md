...
綁定清單會儲存在裝置的持久化空間；一般重新開機不會清除，節點暫時離線也不會自動解除綁定。只有在 `TraceRoute` 清單中長按解除，或清除／重置裝置儲存資料時才會移除。
...
短按送出後，HermesX 會顯示既有 TraceRoute popup，先提示正在等待回應，收到結果後顯示路由與訊號資訊。等待回應上限為 30 秒；如果超過 30 秒仍未收到符合目標節點與 request ID 的回覆，會顯示 `等待回應逾時`。
