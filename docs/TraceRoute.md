...
進入 `搜尋裝置` 會叫出沿用 `GROUP PIN` 配置的鍵盤，並另外提供 `EXIT` 退出鍵。輸入目標節點的 `ShortName` 後選擇 `OK`，HermesX 會關閉鍵盤並以置中小視窗顯示搜尋結果；ShortName 比對不分英文字母大小寫。

找到節點時，小視窗會顯示該節點的 `ShortName` 與 `LongName`，按下 `返回` 後回到綁定清單，游標會停在符合的節點。找不到符合的在線節點時，小視窗會顯示 `找不到裝置` 與剛才輸入的 ShortName，按下 `返回` 回到綁定清單。
...
| 頁面 | 操作 | 行為 |
| --- | --- | --- |
| `綁定節點` | `搜尋裝置` 輸入 ShortName 後選擇 `OK` | 定位到符合的在線節點 |
