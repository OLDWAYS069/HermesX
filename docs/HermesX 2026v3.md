...
目前這份文件對應產品版號 `HXB_C0.4.0` 的 `CIV` 版：關閉 EMAC / EM UI，但保留 GROUP、尋人模組、ONLINE、TAK MODE、Fast Setup 與更新模式。
...
`GROUP` 是 HermesX C0.4.0 裡最重要的任務分組概念。
主選單進入 `GROUP` 後會先看到：
...
HermesX 顯示版號目前使用 `HXB_C0.4.0`；OTA / 韌體檔名會帶有建置時間，例如：

```text
HXB_C0.4.0_YYYYMMDD_HHMM
```
