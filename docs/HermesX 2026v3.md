...
目前這份文件對應 `HermesX_G0.3.2` 的 `GOV` 版：由 `HermesX_0.2.9` 功能基底升版，保留 EMAC / EM UI、GROUP、尋人模組、ONLINE、TAK MODE、Fast Setup 與更新模式。

:::warning
`GOV` 版保留本機 EMAC 進入、遠端 `ACTIVATE: EMAC`、`@EmergencyActive`、`STATUS: LOST` 與 Lighthouse sleep 控制。
如果你需要關閉 EMAC 的民用設定，請確認使用的是 CIV 分支與對應韌體。
:::
...
`GROUP` 是 HermesX G0.3.2 裡最重要的任務分組概念。
主選單進入 `GROUP` 後會先看到：
...
在 GOV 版中，`解除EMAC` 保留為 EMAC 操作與復原流程的一部分。

`節點列表` 顯示同組裝置的 presence。只要設定 GROUP PIN，GOV 版也會透過 GROUP Heartbeat 維持同組節點狀態，不需要進入 EMAC。
...
在 GOV 版中，EMUI 與 EMAC 入口保留；TAK 設定中的 EMUI 入口可用於進入相關緊急介面。
...
`HermesX_G0.3.2` 是 GOV 版，因此功能邊界如下：
...
| EMAC / EM UI 本機進入 | 啟用 |
| 遠端 `ACTIVATE: EMAC` / `@EmergencyActive` | 啟用 |
| `STATUS: LOST` 週期保護與 Lighthouse sleep 控制 | 啟用 |

簡單說，GOV 版保留「裝置端操作、尋人、同組 presence、節點瀏覽與更新」，並保留完整 EMAC / EM UI 緊急接管流程。
...
HermesX 顯示版號目前使用 `HXB_G0.3.2`；OTA / 韌體檔名會帶有建置時間，例如：

```text
HXB_G0.3.2_YYYYMMDD_HHMM
```
