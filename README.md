...
> 目前此分支為 `HermesX_G0.3.2` 的 `GOV` 版：由 `HermesX_0.2.9` 原樣升版，保留 GOV 行為與 EMAC/Lighthouse 功能。
...
- 目前 `HermesX_G0.3.2` 為 `GOV` 版：由 `HermesX_0.2.9` 原樣升版，預設保留 `EMAC/Lighthouse`，不套用 `CIV` 的關閉配置。
