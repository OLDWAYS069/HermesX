...
> 目前此分支為 `HermesX_C0.3.2` 的 `CIV` 版：由 `HermesX_0.2.9` 原樣升版，關閉 EMAC，但保留 GROUP 與尋人模組。
...
- 目前 `HermesX_C0.3.2` 為 `CIV` 版：由 `HermesX_0.2.9` 原樣升版；預設關閉 EMAC 進入與遠端 EMAC 指令，但保留 `GROUP/Lighthouse` 的 GROUP PIN 與尋人位置脈衝能力。
