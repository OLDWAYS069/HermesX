...
// Validate UTF-8 and reject control characters (except newline)
static bool isPrintableUtf8(const char *s, size_t maxBytes)
{
    const unsigned char *p = reinterpret_cast<const unsigned char *>(s);
    const unsigned char *end = p + maxBytes;
    while (p < end && *p) {
        unsigned char c = *p;
        if (c < 0x20 && c != '\n' && c != '\r' && c != '\t') {
            return false;
...
static lodb_uuid_t usernameToUuid(const char *username, uint32_t hostNodeId, bool legacyLowercase)
{
    char normalized[LOBBS_USERNAME_BUFFER_SIZE];
    size_t len = strnlen(username, LOBBS_USERNAME_BUFFER_SIZE - 1);
    memcpy(normalized, username, len);
    normalized[len] = '\0';
...
bool LoBBSDal::isValidUsername(const char *username)
{
    ...
    if (len == 0 || len > LOBBS_MAX_USERNAME_LEN) {
        return false;
    }
    return isPrintableUtf8(username, len);
}
...
bool LoBBSDal::isValidPassword(const char *password)
{
    ...
    return isPrintableUtf8(password, len);
}
...
{
    lodb_uuid_t userUuid = usernameToUuid(username, hostNodeId, false);
    LoDbError err = db->get("users", userUuid, user);
    if (err == LODB_OK) {
        LOG_DEBUG("Loaded user by username: %s", username);
        return true;
    }
    // Fallback for legacy lowercase hashing
    userUuid = usernameToUuid(username, hostNodeId, true);
    err = db->get("users", userUuid, user);
    if (err == LODB_OK) {
        LOG_DEBUG("Loaded user by username (legacy uuid): %s", username);
        return true;
    }
...
lodb_uuid_t userUuid = usernameToUuid(username, hostNodeId, false);
    ...
    return loginUser(username, nodeId, userUuid);
}
...
bool LoBBSDal::loginUser(const char *username, uint32_t nodeId, uint64_t knownUuid)
{
    // Create session record
    meshtastic_LoBBSSession session = meshtastic_LoBBSSession_init_zero;
    session.user_uuid = knownUuid ? knownUuid : usernameToUuid(username, hostNodeId, false);
...
{
    uint64_t userUuid = usernameToUuid(username, hostNodeId, false);
    meshtastic_LoBBSUser user = meshtastic_LoBBSUser_init_zero;
    LoDbError err = db->get("users", userUuid, &user);
    if (err == LODB_OK) {
    ...
    err = db->get("users", userUuid, &user);
    if (err == LODB_OK) {
        LOG_DEBUG("Found legacy user UUID for %s: " LODB_UUID_FMT, username, LODB_UUID_ARGS(userUuid));
        return userUuid;
    }
    LOG_DEBUG("User not found: %s", username);
    return 0;
